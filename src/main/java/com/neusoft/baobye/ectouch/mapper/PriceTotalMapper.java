...
public interface PriceTotalMapper extends JpaRepository<PrizeTotal,Long> {
    ...
    @Query(value="SELECT A.*,B.wechat,B.tel,B.username,B.name FROM prize_total A ,wap_user B WHERE A.USER_ID = B.USER_ID AND A.USER_ID = ?1 AND A.STATUS = ?2 ",nativeQuery = true)
